...
The first two sessions covered the costs, budgets, and revenue streams for data repositories. Repositories have a wide range of pricing and revenue structures. The witness statements in the session link above provide details on the pricing models. Repositories are using a mix of subscriptions (paid by libraries, institutions, researchers), pay to download (to get specific datasets), or pay to deposit (pay to upload to the repository), and external funding (government, private).

While each pricing model has its own strengths and downsides, the loser is clearly academic institutions, libraries, and researchers. Library managers and researchers expressed concern that they will be unable to keep up financially with the number of data repository subscriptions they need to access data. These costs are added onto the high costs institutions pay for journal subscriptions. Additionally, as more data becomes replicated in several places, it becomes unclear which repositories institutions should subscribe to.

From the perspective of Dat, the costs were the most interesting part. Hosting and bandwidth for the data repositories amount to 1/4th to 1/3rd of their total costs. We hope that Dat will be able to better distribute or reduce a lot of those costs.

Repositories that offer *active* dataset curation also had a high cost for the data curators, a new profession, at around 1/3 of the total costs.

The last big chunk of spending was on software development and general research and development. This is not surprising, software is expensive. But unfortunately, repositories were duplicating the same costs developing similar software.
...
The [USGS data management plan](https://www2.usgs.gov/datamanagement/why.php) is a well thought out plan on each stage of the data life-cycle, how the USGS is approaching it, and summaries of the federal requirements around data.
...
The presenters made clear that open data is a public good, such as in cases like natural disaster prevention. It is unreasonable to say all research data must be totally open. Fully open data creates privacy concerns, IP issues, and technological hurdles. Making specific recommendations, such as around reproducibility of published results, can go a long way in supporting both the public good and research needs.
...
Research Data Alliance is a collection of academics, librarians, and other groups involved with research data. RDA has a [good primer](https://rd-alliance.org/rda-newcomers-some-notes) on their organization structure. RDA members meet with a focus on interest groups and working groups. Working groups and interest groups continue to communicate throughout the year to form recommendations (working groups) or help inform discussion throughout RDA (interest groups).

### Session Notes

Specific sessions notes collected below.
...
* Libraries will provide for a key partner in managing and publishing long tail data.
...
* "No tool will be adopted unless there is economic incentive (enlightened self-interest)"
* What do we mean by reproducibility? Trying to get the same answer or independently implement the same methodology to see what answer if given. There are researchers reproducing others' work, usually students, postdocs but there is no way to publish or credit them. We know they are happening But not what happens or who is doing them. We only hear about them when they differ significantly from the original result.
...
* How to do data versioning with co-location with petascale computers and cloud intensive computing?
* This is also an issue for small datasets.
* query sub-setting data is important; spatial queries useful
...
* **Battle over who assigns the DOI/PID for dataset and subsets of data**. If data is of value, the institution or repository owner will store it. But who mints the DOI?
...
There are several RDA interest groups and working groups have specific recommendations out which may overlap with Dat:
...
There are also RDA working groups and interest groups that are in progress, i.e. formalizing their recommendations, that overlap with Dat:
...
* Tons of discussion across many groups of metadata, standards, and interoperability. This is a huge problem. There are many many (way more than I thought) specific domain or more generalized data repositories. Some of these have common metadata (usually in domain specific case) but many data repositories have their own and rely on translations to/from other metadata formats in order to improve interoperability.
